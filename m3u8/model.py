...
class M3U8(object):
    ...
    def dumps(self):
        ...
        You could also use str(<this obj>)
        '''
        output = ['#EXTM3U']
        if self.media_sequence is not None and self.media_sequence > 0:
...
class Key(BasePathMixin):
    ...
    '''
    def __init__(self, method, uri=None, base_uri=None, iv=None):
        self.method = method
        self.uri = uri
        self.iv = iv
        self.base_uri = base_uri
...
class Playlist(BasePathMixin):
    ...
    def __init__(self, uri, stream_info, media, base_uri):
        ...
        for media_type in ('audio', 'video', 'subtitles'):
            ...
            self.media += list(filter(lambda m: m.group_id == group_id, media))
...
def _urijoin(base_uri, path):
    if parser.is_url(base_uri):
        ...
        new_path = posixpath.normpath(os.path.join(parsed_url.path, '..', path))
        return url_parser.urljoin(prefix, new_path)
    else:
