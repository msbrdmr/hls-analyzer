...
def _parse_key(line):
    ...
    for param in params:
        ...
        key[normalize_attribute(name)] = remove_quotes(value) if value else None
    return key

def _parse_extinf(line, data, state):
    if ',' in line:
        duration, title = line.replace(protocol.extinf + ':', '').split(',', 1)
    else:
        duration = line.replace(protocol.extinf+ ':', '')
        title = ''
...
def string_to_lines(string):
    if isinstance(string, bytes):
        return string.decode().strip().replace('\r\n', '\n').split('\n')
...
def remove_quotes(string):
    ...
    quotes = ('"', "'")
    if string and string.startswith(quotes) and string.endswith(quotes):
        return string[1:-1]
    return string
