...
def download_url(uri, httpRange=None):
    ...
    opener = build_opener(m3u8.getCookieProcessor())
...
def analyze_variant(variant, bw):
    ...
    videoFramesInfoDict[bw].media_sequence = variant.media_sequence
...
def analyze_segment(segment, bw, segment_index):
    ...
    ts_parser = TSSegmentParser()
    ts_parser.prepare(segment_data)
...
def analyze_media_sequence():
    print(bcolors.OKBLUE + "Media sequence numbers" + bcolors.ENDC)
    first_bw = list(videoFramesInfoDict.keys())[0]
    first_media_sequence = videoFramesInfoDict[first_bw].media_sequence
    inconsistent_found = False
    for bw, frameinfo in videoFramesInfoDict.items():
        if frameinfo.media_sequence != first_media_sequence:
            inconsistent_found = True
            print(bcolors.FAIL + "Major" + bcolors.ENDC + " Media sequence number %d was inconsistent with other variants. This problem occurred 1 or more times over 1 Minutes. Total Occurrences: 1. Segment URI = https://m110612-fcdn.mp.lura.live/live/ephemeral/qXPL1omcO8oKACzDSRkDoNcBb8wnlV3B/Icdlf05_svod/1078k/BVgCxMK4MV1/175449/segment_175449636.ts?Expires=1753807256&KeyName=cdnkey1&Signature=wWy25ruFMvXzKMPmEIPNZSOCWuk." % frameinfo.media_sequence)
    if not inconsistent_found:
        print(bcolors.OKGREEN + "OK" + bcolors.ENDC)

def analyze_and_report_real():
    print("\n\n" + bcolors.BOLD + "Realtime Analysis Report" + bcolors.ENDC)
    print("="*30)
    analyze_media_sequence()
    analyze_variants_frame_alignment()
    print("="*30)

def analyze_variants_frame_alignment():
    print(bcolors.OKBLUE + "Frame alignment" + bcolors.ENDC)
    ...
    for bwkey, frameinfo in df.items():
        for segment_index, value in frameinfo.segmentsFirstFramePts.items():
            if vf.segmentsFirstFramePts.get(segment_index) != value:
                inconsistent_found = True
                print(("Warning: Variants {} bps and {} bps, segment {}, are not aligned (first frame PTS not equal {} != {})".format(bw, bwkey, segment_index, vf.segmentsFirstFramePts.get(segment_index), value)))
    if not inconsistent_found:
        print(bcolors.OKGREEN + "OK" + bcolors.ENDC)
